...
//   Bot propio (TELEGRAM_BOT_TOKEN_LOLA). Flyers/artes/videos con fal.ai vía scripts/fal.mjs
//   (FAL_API_KEY; desde el 25/sep, antes Higgsfield) y guiones escritos al momento; todo a la bandeja de
//   Entregas. Si fal falla, encola el pedido en data/pedidos-lola.json y la tarea
...
function correrClaude(prompt, persona, sesion, nueva, onProgreso, opts = {}) {
  return new Promise((resolve) => {
    ...
    else if (ES_MAX) args.push("--permission-mode", "default", "--allowedTools", "Read", "Glob", "Grep", "Bash(node scripts/meta-ads.mjs*)", "Bash(node scripts/fal.mjs*)", "Bash(node scripts/agentes.mjs*)", "Bash(node scripts/max.mjs*)", "--disallowedTools", "Edit", "Write", "WebFetch", "WebSearch");
...
// ── Max vive en Slack (Elvin, 24/sep/2026) ─────────────────────────────────────────────────────
// Lo que llega de Slack (cliente escribió en su canal, Elvin/Carilin decidieron o comentaron en
// #max-aprobaciones, onboarding nuevo) entra al buzón como de: "slack". Max lo trabaja con su
// cerebro §17 y responde SOLO con scripts/max.mjs (proponer / nota / cerrar): nunca al cliente directo.
const MAX_SLACK = `OPERAS EN SLACK (cerebro-max.md §17 — léelo si no lo tienes fresco). Reglas duras:
- NUNCA le escribes al cliente directo. Todo lo que el cliente deba ver (respuesta, pedido de material, plan de marketing, creativos) lo PROPONES: node scripts/max.mjs proponer <slug> mensaje|plan|creativos --titulo '…' --texto '…' [--hilo <ts del mensaje del cliente>]. Cuando Elvin o Carilin dicen ok, el servidor se lo manda tal cual. Escribe el texto FINAL para el cliente (tuteo PR, firma — Max, Level Up Media), nada de notas internas dentro.
- Campañas: primero propones la estructura (proponer <slug> campana). Con el ok, la montas EN PAUSA con node scripts/meta-ads.mjs cliente:<slug> … y pides publicar con node scripts/meta-ads.mjs cliente:<slug> proponer-publicar <campaignIds> --cliente <slug>. SOLO si te llega 'evento publicar' corres node scripts/meta-ads.mjs cliente:<slug> activar --item <id>. Nunca actives nada de otra forma.
...
async function main() {
  ...
  // Nico en la nube: la memoria de Claude de la Mac (claude-memoria/, sync cada 15 min) es su memoria
  // automática. Mismo enlace que scripts/nico-nube.sh, aquí para que baste un reinicio.
  if (EN_NUBE && ES_NICO) {
    try {
      const mem = path.join(process.env.HOME, ".claude", "projects", ROOT.replace(/[/ ]/g, "-"), "memory");
      fs.mkdirSync(path.join(ROOT, "claude-memoria"), { recursive: true });
      fs.mkdirSync(path.dirname(mem), { recursive: true });
      if (!fs.existsSync(mem) || !fs.lstatSync(mem).isSymbolicLink()) { fs.rmSync(mem, { recursive: true, force: true }); fs.symlinkSync(path.join(ROOT, "claude-memoria"), mem); }
      LOG("memoria de Claude enlazada:", fs.readdirSync(path.join(ROOT, "claude-memoria")).length, "notas");
