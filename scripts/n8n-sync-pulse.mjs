...
// existenteId: ya está en Chatwoot pero en otro buzón (p. ej. Setters, inbox 12) → se le agrega el 6.
return { json: { contactId: c && ci ? c.id : null, sourceId: ci ? ci.source_id : null, existenteId: c && !ci ? c.id : null } };` }, pos(9, 0)),
    ...
    nodo("¿En otro buzón?", "n8n-nodes-base.if", 2.2, { conditions: { options: opts, conditions: [cond("={{ !!$json.existenteId }}", "", "true", "boolean")], combinator: "and" }, options: {} }, pos(11, 0)),
    nodo("Agregar al WhatsApp", "n8n-nodes-base.httpRequest", 4.2, { method: "POST", url: `=${CHATWOOT}/contacts/{{ $json.existenteId }}/contact_inboxes`, authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendBody: true, specifyBody: "json", jsonBody: "={{ JSON.stringify({ inbox_id: 6 }) }}", options: {} }, pos(12, 0), { credentials: { httpHeaderAuth: CRED_CW } }),
    nodo("Contacto agregado", "n8n-nodes-base.code", 2, { mode: "runOnceForEachItem", jsCode: "return { json: { contactId: $('Elegir contacto').item.json.existenteId, sourceId: $json.source_id || ($json.payload && $json.payload.source_id) || null } };" }, pos(13, 0)),
    nodo("¿Calendario?", "n8n-nodes-base.if", 2.2, { conditions: { options: opts, conditions: [cond("={{ $json.sinBienvenida !== true }}", "", "true", "boolean")], combinator: "and" }, options: {} }, pos(5, -2)),
    nodo("Crear contacto", "n8n-nodes-base.httpRequest", 4.2, { method: "POST", url: `${CHATWOOT}/contacts`, authentication: "genericCredentialType", genericAuthType: "httpHeaderAuth", sendBody: true, specifyBody: "json", jsonBody: "={{ JSON.stringify({ inbox_id: 6, name: $('Datos contacto').item.json.nombreCompleto, email: $('Datos contacto').item.json.email || undefined, phone_number: $('Datos contacto').item.json.telefono ? '+' + $('Datos contacto').item.json.telefono : undefined, identifier: $('Datos contacto').item.json.telefono + '@s.whatsapp.net', custom_attributes: { '10dias': false, '30dias': false } }) }}", options: {} }, pos(12, 1), { credentials: { httpHeaderAuth: CRED_CW } }),
    nodo("Contacto creado", "n8n-nodes-base.code", 2, { mode: "runOnceForEachItem", jsCode: `const p = $json.payload || {};
return { json: { contactId: p.contact ? p.contact.id : null, sourceId: p.contact_inbox ? p.contact_inbox.source_id : null } };` }, pos(13, 1)),
    nodo("Contacto listo", "n8n-nodes-base.code", 2, { mode: "runOnceForEachItem", jsCode: "return { json: { contactId: $json.contactId, sourceId: $json.sourceId } };" }, pos(14, 0)),
    ...
    // Solo cuenta una conversación del buzón de WhatsApp (6): una de Setters (12) no le llega al cliente.
    nodo("¿Sin conversación?", "n8n-nodes-base.if", 2.2, { conditions: { options: opts, conditions: [cond("={{ !(($json.payload) || []).some((c) => c.inbox_id === 6) }}", "", "true", "boolean")], combinator: "and" }, options: {} }, pos(15, 0)),
  ...
  const connections = {
    ...
    "¿Crear?": { main: [[to("¿Calendario?")], [to("¿Simulación?")]] },
    "¿Calendario?": { main: [[to("Evento en Calendar")], [to("Fila en onboarding")]] },
    ...
    "¿Existe en Chatwoot?": { main: [[to("Contacto listo")], [to("¿En otro buzón?")]] },
    "¿En otro buzón?": { main: [[to("Agregar al WhatsApp")], [to("Crear contacto")]] },
    "Agregar al WhatsApp": { main: [[to("Contacto agregado")]] },
    "Contacto agregado": { main: [[to("Contacto listo")]] },
