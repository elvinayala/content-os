...
export async function activarAction(formData: FormData) {
  ...
  const leads = formData.get("d") === "leads";
  const volver = (e: string) => redirect(`/ritmo/activar?t=${encodeURIComponent(t)}${leads ? "&d=leads" : ""}&error=${e}`);
  ...
  redirect(leads ? "/pulse/leads" : "/ritmo");
}
