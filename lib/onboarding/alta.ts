...
import { PREGUNTAS, type Pregunta, type Respuestas, texto, visible } from "./level-up";

// Formulario propio de Level Up → ficha del cliente en Pulse.
export async function altaDesdeFormulario(token: string, r: Respuestas, opciones: { tablero?: string; preguntas?: Pregunta[] } = {}): Promise<ResultadoAlta & { resumen: string; negocio: string; email: string }> {
  const preguntas = opciones.preguntas ?? PREGUNTAS;
