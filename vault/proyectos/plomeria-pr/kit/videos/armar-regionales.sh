#!/bin/bash
# Reel de clientes POR REGIÓN (18 s, 9:16, con música): el video de 15 s + 3 s de cierre con la historia
...
for slug in ${1:-metro bayamon caguas ponce arecibo mayaguez aguadilla fajardo}; do
  ffmpeg -v error -y -i "$AQUI/resuelto-clientes-15s.mp4" -i "$FLY/cliente-$slug-story.png" -i "$PISTA" \
    -filter_complex "[1:v]scale=1080:1920,zoompan=z='min(1.0+0.0008*on,1.06)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=90:s=1080x1920:fps=30,fade=t=in:st=0:d=0.3,setsar=1[fin];[0:v]setsar=1[v0];[v0][fin]concat=n=2:v=1:a=0[v]" \
