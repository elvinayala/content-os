# Pista de sonido del reel de plomeros (15 s), compuesta en código y sincronizada a los cortes:
# escenas en 0 · 3.2 · 6.5 · 10.6 · 13.4 · contador $1,950 de 7.0 a 8.7 · ticks 10.8/11.15/11.5 · logo 13.4
import numpy as np, wave, sys
SR = 44100; DUR = 15.0; N = int(SR*DUR); t = np.arange(N)/SR
out = np.zeros(N)
...
for cut in (3.2, 6.5, 10.6, 13.4):
    w=whoosh(0.55); place(w, cut-0.55, 0.75)
# ── Contador $0 → $1,950 (7.0 → 8.7): ticks acelerando
tt=7.0; step=0.11
while tt<8.7:
    place(tick(f=1600+400*((tt-7)/1.7)), tt, 0.5); step=max(0.045, step*0.93); tt+=step
place(pop(), 8.72, 0.9)  # cierra el contador
# ── Checks verdes
for c in (10.8, 11.15, 11.5): place(pop(), c+0.3, 0.85)
# ── Remate del logo + fade de todo
place(kick(), 13.4, 1.0); place(stinger(), 13.4, 1.0)
fade_start=int(13.9*SR); out[fade_start:] *= np.linspace(1,0,N-fade_start)**1.5
